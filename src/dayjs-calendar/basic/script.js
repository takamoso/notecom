...
const generate = (number = 2) => {
  ...
  for (let i = 0; i < number; i++) {
    const target = now.add(i, 'month')                    // 該当月
    const start = target.startOf('month')                 // 月初の日付
    const end = target.endOf('month')                     // 月末の日付
    const endDate = end.get('date')                       // 月末の日にち
    const lastEndDate = start.add(-1, 'day').get('date')  // 先月末の日にち
    const startWeekday = start.get('day')                 // 月初の曜日
    const endWeekday = end.get('day')                     // 月末の曜日

    calendar += '<table class="c-calendar">'
    
    // 年月を表示
    calendar += `<caption class="c-calendar_title"><span data-year="${target.format('YYYY')}">${target.format('M')}</span></caption>`
    ...
    for (let date = 1; date <= endDate; date++) {
      ...
      calendar += `<td class="c-calendar_date${holiday.includes(current.format('YYYYMMDD')) ? ' -holiday' : ''}"><span>${date}</span></td>`
